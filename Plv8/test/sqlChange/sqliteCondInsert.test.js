...
test('Update multiple entities with condition test', () =>
{
    ...
    const items = sqlite.run(`SELECT * FROM ${tableName} WHERE id IN (5, 6)`);
    ...
    expect(item.name).toBe('Animals');
    expect(item.account_id).toBe(1);
    expect(item.company_type_id).toBe(2);
