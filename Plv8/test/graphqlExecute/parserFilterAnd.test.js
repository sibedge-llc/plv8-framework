...
test('Mock, Two filder conditions should be used', () =>
{
    ...
    expect(where.right.operator).toBe('ILIKE');  
    expect(where.right.left.type).toBe("column_ref");
    expect(where.right.left.column).toBe("name");
    expect(where.right.right.type).toBe('single_quote_string');
    expect(where.right.right.value).toBe('A%');    
});
